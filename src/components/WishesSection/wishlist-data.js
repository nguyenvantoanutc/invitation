import imgSon from './assets/w-son-ba-hai.jpg';
import imgTinh from './assets/w-mai-tinh.jpg';
import imgDan from './assets/w-linh-dan.jpg';
import imgQuyen from './assets/w-pham-quyen.jpg';
import imgThanhHa from './assets/w-thanh-ha.jpg';
import imgKimEn from './assets/w-kim-en.jpg';
import imgBichQuy from './assets/w-bich-quy.jpg';
import imgVanLuc from './assets/w-van-luc.jpg';
import imgGirl from './assets/girl.png';
import imgThuHa from './assets/w-ha.jpg';
import imgHongHa from './assets/w-hong-ha.jpg';
import imgVinhTB from './assets/w-vinh-tb.jpg';
import imgLeChi from './assets/w-le-chi.jpg';

export const wishlist = [
    {
        name: 'Sơn Ba Hai',
        infoName: 'SVMC - Samsung',
        fbLink: "https://www.facebook.com/sonsoyaa",
        description: `Chúc mừng tân lang - tân nương điển trai xinh đẹp của tớ. Chúc 2 cậu sẽ có một khởi đầu mới thuận lợi, cùng nhau xây đắp một mái ấm hạnh phúc mà hai cậu vẫn ao ước nhé. Happy Wedding!`,
        image: imgSon,
    },
    {
        name: 'Nguyễn Văn Lực',
        infoName: 'NewWave Solution',
        fbLink: "https://www.facebook.com/nguyenlucbg94",
        description: `Thật vui vì cuối cùng ngày ấy cũng tới. Một chương mới của cuộc đời đã mở ra với hai bạn. Tôi mong chặng đường về sau sẽ luôn tràn ngập niềm vui, tiếng cười và hành phúc.
                        Và Tôi tin rằng đây sẽ là khởi đầu cho những điều tốt đẹp sắp tới trong cuộc sống của hai bạn. Hãy yêu thương và cùng nhau vượt qua mọi khó khăn trong cuộc sống nhé<br/>
                        Cuối cùng thì hãy nhớ rằng chuỗi ngày điên cuồng, mệt mỏi sắp bắt đầu với bạn rồi. Hãy vượt qua khó khăn này nhé 😀`,
        image: imgVanLuc,
    },
    {
        name: 'Anh Hà',
        infoName: 'ABC Yên Trung',
        fbLink: "https://www.facebook.com/hanosimex.ha",
        description: `Người xưa có nói "Tu 10 năm mới chung chuyến đò, tu 100 năm mới chung chăn gối". Đến với nhau là do duyên phận, còn hạnh phúc đến trọn đời do hai người cùng vun đắp. Các em hãy cùng nhau xây dựng một tổ ấm và bên nhau đến "Đầu bạc răng long" mà thôi phải nói là "Tóc nhuộm răng giả" chứ bây giờ già vẫn cần đẹp :D nhỉ
                        Hãy nhớ câu "Thuận vợ thuận chồng tát biển Đông cũng cạn". <br/>
                        Chúc đôi uyên ương có một lễ cưới trang trọng, đầm ấm đánh dấu bước khởi đầu của con đường Hạnh Phúc`,
        image: imgHongHa,
    },
    {
        name: 'Thanh Hà',
        infoName: 'VNPT VAS',
        fbLink: "https://www.facebook.com/ha.zu.58",
        description: `Hôn nhân ko phải là nơi thuyền tình cập bến, mà là nơi 2 người yêu nhau quyết định cùng giăng buồm vượt sóng ra khơi”. Chúc mừng TOÀN đã không còn đơn PHƯƠNG trong hành trình hạnh phúc này!`,
        image: imgThanhHa,
    },
    {
        name: 'Nguyễn Thu Hà',
        infoName: 'ABC Yên Trung',
        fbLink: "https://www.facebook.com/thuha.nguyen.104",
        description: `Chúc Toàn ❤ Phương về chung 1 nhà. Không chúc đầu bạc răng long, nhỡ 35-40t tóc bạc răng rụng thì k ổn nên chúc sống bên nhau trọn đời trọn kiếp và thật hạnh phúc nha 2 đứa :D`,
        image: imgThuHa,
    },
    {
        name: 'Mai Tình',
        infoName: 'Viet Solution',
        fbLink: "https://www.facebook.com/eldy968",
        description: `Chúc cho Toàn Phương nên đôi vợ chồng <br/>
                        Chúc cho hai bạn mặn nồng tình thương <br/>
                        Thương nhau thương tới tận già<br/>
                        Thương cho tới lúc bạc cha mái đầu.`,
        image: imgTinh,
    },
    {
        name: 'Nhị Susan',
        infoName: 'Bạn Mr. Toàn',
        fbLink: "https://www.facebook.com/susan120796",
        description: `Thật vui vì ngày này cũng đã đến, bạn t đã dừng chân ở bến đỗ, mảnh ghép hoàn hảo của cuộc đời !!!<br/>
                        Chúc cho hôn lễ của đôi trai tài gái sắc diễn ra thật ý nghĩa và tốt đẹp.<br/>
                        Chúc cho cuộc sống mới của 2 bạn luôn tràn đầy niềm vui, tiếng cười và hạnh phúc. Và dù có bao nhiêu thử thách, hãy luôn nắm chặt tay, cùng nhau đi hết cuộc đời nhé !!!<br/>
                     HAPPY WEDDING`,
        image: imgGirl,
    },
    {
        name: 'Kim Én',
        infoName: 'ABC Yên Trung',
        fbLink: 'https://www.facebook.com/kim.en.33',
        description: `Chúc mừng hạnh phúc hai em! Chúc hai em có một đám cưới trọn vẹn niềm vui, có một cuộc sống như ý. “Hôn nhân không phải là nơi thuyền tình cập bến mà là nơi hai người yêu nhau quyết định cùng giăng buồm vượt sóng ra khơi”. Chúc hai em căng buồm ra khơi thuận buồm xuôi gió.`,
        image: imgKimEn,
    },
    {
        name: 'Quy Nguyễn',
        infoName: 'Bạn Mrs. Phương',
        fbLink: 'https://www.facebook.com/nguyenthibichquy.96',
        description: `Chúc mừng hạnh phúc hai bạn và gia đình! Mình cảm thấy thật may mắn khi chúng ta là bạn của nhau và hôm nay mình lại có cơ hội được đứng ở đây để chúc phúc cho cuộc hôn nhân của bạn! <br/>Thật ra mình cũng  không biết chúc gì hơn, chỉ chúc hai bạn trong cuộc sống lúc nào cũng thuận vợ, thuận chồng. Chúc hai bạn luôn hạnh phúc, đầu bạc răng long, con cái đề huề! Happy wedding!<br/>Phải thật hạnh phúc nhé bạn thân!`,
        image: imgBichQuy,
    },
    {
        name: 'Linh Đan',
        infoName: 'Viet Solution',
        fbLink: 'https://www.facebook.com/do.dan.50',
        description: `Gửi anh Toàn, <br/> Vậy là anh em mình đã quen nhau 5 năm rồi. Đi nhậu cũng nhiều, cứ tưởng rằng trong nhóm lầy lội a sẽ cưa đổ được chị nào nhưng hoá ra không phải, anh tìm một nửa của mình là chị Phương gần nhà mà chưa từng ra mắt nhóm. Chúc cho anh chị mãi mãi hạnh phúc, cùng nắm tay nhau xây dựng một gia đình nhiều niềm vui, ấm áp nha! Thi thoảng tổ chức nhậu gọi team đến oánh chén :D`,
        image: imgDan,
    },
    {
        name: 'Nguyễn Trọng Vinh',
        infoName: 'SVMC - Samsung',
        fbLink: 'https://www.facebook.com/nguyentrongvinh.tb.96',
        description: `Chúc mừng hạnh phúc lứa đôi.. Tớ mong 2 bạn sẽ có cuộc hôn nhân viên mãn, đong đầy tình cảm, xây dựng một tổ ấm đầy ắp tiếng cười trẻ thơ và chan hòa yêu thương .. Cung hỉ, cung hỉ...`,
        image: imgVinhTB,
    },
    {
        name: 'Phương Chi Lê',
        infoName: 'Chị Mrs. Phương',
        fbLink: 'https://www.facebook.com/phuongchi.le.94',
        description: `Congratulations on your wedding, sister. You now has someone with you to go to the end of your life. Wish you always happy and happy with your wonderful husband`,
        image: imgLeChi,
    },
    {
        name: 'Phạm Quyền',
        infoName: 'VNPT VAS',
        fbLink: 'https://www.facebook.com/phamquyen29',
        description: `Hôn nhân chính là khoảng thời gian đẹp nhất để cả hai tâm hồn có thể chia sẻ với nhau. Hai bạn hãy thắp sáng tình yêu bằng những ngọn nến yêu thương nhau mỗi ngày. Chúc mừng hạnh phúc đôi bạn trẻ!`,
        image: imgQuyen,
    },
];
